	...
	def _prepare_move_line_default_vals(self, write_off_line_vals=None):
		...
		for rec in self:
			if rec.tax_type.is_withholding:
				...
				# Compute amounts.
				write_off_amount_currency = write_off_line_vals.get('is_withholding_amount', 0.0)

				if self.payment_type == 'inbound':
					# Receive money.
					liquidity_amount_currency = self.is_withholding_amount
		...
		for rec in self:
			if rec.tax_types.is_withholding:
				payment_vals = {
					...
					'amount': self.is_withholding_amounts,
				...
				res = super(AccountPaymentRegisterInherit, self)._create_payment_vals_from_wizard()
				return res
